...
def make_new_example(eid=None, source=None, text=None, sentence_id=None, claim=None, label=None, extraction_span=None,
...
class SampleSentences(Transformation):
    ...
    def __init__(self, num_samples=8, min_sent_len=8):
        super().__init__()
    ...
    def transform(self, example):
        ...
        sents = [sent for sent in page_doc.sents if len(
            sent) >= self.min_sent_len]

        # sample claims
        claims = random.sample(sents, min(self.num_samples, len(sents)))
        new_examples = []
        for i, claim in enumerate(claims):
            new_example = make_new_example(eid=page_id, source=example['source'],
                                           text=page_doc,  # avoid repeating the same text for all claims
                                           sentence_id=i,
                                           claim=self.spacy(claim.text),
                                           label=LABEL_MAP[True],
                                           extraction_span=(
                                               claim.start, claim.end-1),
...
class NegateSentences(Transformation):
    ...
    def __negate_sentences(self, claim):
        ...
        if is_negative:
            if claim[negated_ix + 1].text.lower() == "n't":
                if claim[negated_ix + 1].text.lower() == "ca":
                    claim_tokens[negated_ix] = "can" if claim_tokens[negated_ix].islower(
                    ) else "Can"
            ...
            if negation == "n't ":
                if claim[negated_ix].text.lower() == "can":
                    claim_tokens[negated_ix] = "ca" if claim_tokens[negated_ix].islower(
                    ) else "Ca"
                else:
        ...
        augmentation_span = (
            negated_ix, negated_ix if is_negative else negated_ix + 1)
...
class Backtranslation(Transformation):
    ...
    def __init__(self, dst_lang=None, api='easynmt'):
        super().__init__()

        self.src_lang = "en"
        self.dst_lang = dst_lang
        self.api = api
        self.accepted_langs = ["fr", "de", "zh-TW", "es",
                               "ru"] if api == "google" else ["fr", "de", "zh", "es", "ru"]
        self.translator = translate.Client() if api == "google" else Translator()

    def transform(self, example):
        if isinstance(example, list):
            new_examples = [dict(eg) for eg in example]
            new_claims, _ = self.__backtranslate([eg["claim"] for eg in new_examples])

            if new_claims:
                for i, new_claim in enumerate(new_claims):
                    new_examples[i]["claim"] = new_claim
                    new_examples[i]["backtranslation"] = True
                return new_examples
            else:
                return None
        else:
    ...
    def __backtranslate(self, claims):
        # chose destination language, passed or random from list
        dst_lang = self.dst_lang if self.dst_lang else random.choice(
            self.accepted_langs)

        lst2translate = [claim.text for claim in claims]

        # translate to intermediate language and back
        claims_trans = self.translator.translate(
            lst2translate, source_language=self.src_lang, target_language=dst_lang, format_="text")
        claims_btrans = self.translator.translate(
            claims_trans["translatedText"], source_language=dst_lang, target_language=self.src_lang, format_="text")

        # create new claim object
        new_claims, augmentation_spans = [], []
        for i, new_claim_txt in enumerate(claims_btrans["translatedText"]):
            if new_claim_txt == claims[i].text:
                continue

            new_claim = self.spacy(new_claim_txt)
            new_claims.append(new_claim)
            augmentation_spans.append((new_claim[0].i, new_claim[-1].i))

        return new_claims, augmentation_spans
...
class PronounSwap(Transformation):
    ...
    def __init__(self, prob_swap=0.5):
        ...
        self.pronoun2class_map = {pronoun: key for (
            key, values) in self.class2pronoun_map.items() for pronoun in values}
        self.pronouns = {pronoun for (
            key, values) in self.class2pronoun_map.items() for pronoun in values}
    ...
    def __swap_pronouns(self, claim):
        ...
        candidate_tokens = [
            token for token in self.class2pronoun_map[chosen_class] if token != chosen_token.text.lower()]
        ...
        swapped_token = swapped_token if chosen_token.text.islower(
        ) else swapped_token.capitalize()
...
class NERSwap(Transformation):
    ...
    def __swap_entities(self, text, claim):
        ...
        claim_ents = [
            ent for ent in claim.ents if ent.label_ in self.categories]
        ...
        candidate_ents = [ent for ent in text_ents if ent.text !=
                          replaced_ent.text and ent.text not in replaced_ent.text and replaced_ent.text not in ent.text]
        ...
        claim_tokens = [token.text_with_ws for token in claim]
        swapped_token = align_ws(
            replaced_ent.text_with_ws, swapped_ent.text_with_ws)
        claim_swapped = claim_tokens[:replaced_ent.start] + \
            [swapped_token] + claim_tokens[replaced_ent.end:]

        # create new claim object
        new_claim = self.spacy("".join(claim_swapped))
        augmentation_span = (replaced_ent.start,
